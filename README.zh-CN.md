...
### "我用 Claude Code / Aider / Cline 指向 DeepSeek 不就行了？"

一个好问题，分层回答。

**Claude Code：基本做不到。** 它硬绑 Anthropic API —— 鉴权头、tool schema
形状、`<thinking>` 块解析都假定是 Claude。要让它跑 DeepSeek，得挂一个像
`claude-code-router` 这样的翻译代理：进 Anthropic 协议，出 DeepSeek token。
结果就是所有 Anthropic-specific 的调优变成额外开销，而所有 DeepSeek-specific
的调优都拿不到（见下表）。OpenAI Codex CLI 换别家后端也是同样的问题 ——
厂商自家 CLI 是为自家模型调的。

**Aider / Cline / Continue：是的，它们支持 DeepSeek。** 你确实能拿到便宜的
token 单价。但你拿不到下表里这些 —— 因为这些工具的循环是为了**通用支持**
所有 LLM 设计的：

| Anthropic / OpenAI 循环假定 | DeepSeek 实际行为 | Reasonix 怎么处理 |
|---|---|---|
| `cache_control` 标记，5 分钟 TTL | 自动前缀缓存，长 TTL | append-only log；前缀永不重写，缓存跨轮持续命中 |
...
缓存稳定性是这里最承重的一块。通用 harness 跨轮重排、summarize、或重建
prompt，DeepSeek 上实测命中率 40-60%；同一负载跑 Reasonix 命中 94.4%
（已 commit transcript 可验证，见下面 benchmark 段）。按 DeepSeek 的单价 ——
$0.07/Mtok 非缓存、约 $0.014/Mtok 缓存命中 —— 50% 和 94% 命中之间的差距
**仅 input 这一侧就大约是 2.5×**。

> Reasonix 不是"又一个 agent CLI"，是**专门为 DeepSeek 的怪癖和定价模型
> 调出来的 agent CLI**。
